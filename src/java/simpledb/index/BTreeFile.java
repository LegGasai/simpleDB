...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		int tupNumMove = (splitLeafPage.getNumTuples()+1)/2;
		...
		BTreeLeafPage right = splitLeafPage.getRightSiblingId()==null? null : (BTreeLeafPage) getPage(tid,dirtypages,splitLeafPage.getRightSiblingId(),Permissions.READ_WRITE);
		if (right != null){
			newLeafPage.setRightSiblingId(right.getId());
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int entryNumMove = (splitInternalPage.getNumEntries()+1)/2-1;
		...
		updateParentPointers(tid,dirtypages,splitInternalPage);
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		entry.setKey(iterator.next().getField(keyField));
